...
`;const On={class:"container"},Fn={id:"my-world",ref:"myWorld2"},Pn=Bt({setup(n){zt(()=>ye(this,null,function*(){yield Vt(),s()}));let e,t,r;function s(){let p=new Ht;const f=new k(-20,15,-15),_=document.getElementById("my-world"),T=i(_);e=o(),t=c(e,_);let w=a();A(w);const x=new le({vertexShader:Sn,fragmentShader:bn,side:me,uniforms:{uTime:{value:0},uMouse:{value:new U(0,0)},uResolution:{value:new U(window.innerWidth,window.innerHeight)}}}),N=u(x);N.position.copy(new k(0,0,0));const C=new St(256),F=new bt(.1,10,C),I=new Ee;I.add(N);const G=new le({vertexShader:Mn,fragmentShader:In,side:me,uniforms:{uTime:{value:0},uMouse:{value:new U(0,0)},uResolution:{value:new U(window.innerWidth,window.innerHeight)},uNoiseTexture:{value:null},uVelocity:{value:.05},uBrightness:{value:.33},uStagger:{value:16}}}),O=d(G);O.position.copy(f),l(O),w.add(O);let P=new Z(new Mt(.1,20,20),new q({color:16711680}));P.position.set(0,10,0),w.add(P);const K=new le({vertexShader:Nn,fragmentShader:Cn,side:It,uniforms:{uTime:{value:0},uMouse:{value:new U(0,0)},uResolution:{value:new U(window.innerWidth,window.innerHeight)}}}),_e=h(K);_e.position.copy(f),w.add(_e),m(w);let ie=[];g(w,ie);function ve(){if(b(T)){const X=T.domElement;e.aspect=X.clientWidth/X.clientHeight,e.updateProjectionMatrix()}let ze=p.getDelta();for(let X=0;X<ie.length;X++)ie[X].update(ze);L.update();let re=p.getElapsedTime(),oe=new U(0,0);F.update(T,I),x&&G&&(x.uniforms.uTime.value=re,x.uniforms.uMouse.value=oe,G.uniforms.uTime.value=re,G.uniforms.uMouse.value=oe,G.uniforms.uNoiseTexture.value=C.texture,K.uniforms.uTime.value=re,K.uniforms.uMouse.value=oe),requestAnimationFrame(ve),T.render(w,e)}document.addEventListener("click",v,!1),requestAnimationFrame(ve)}function i(p){const f=new Nt({antialias:!0,logarithmicDepthBuffer:!0,canvas:p});return f.shadowMap.enabled=!0,f.setClearColor(0,0),f}function o(p=40,f=window.innerWidth/window.innerHeight,_=.1,T=1e3){let w=new De(p,f,_,T);return w.position.copy(new k(0,10,50)),w.lookAt(new k(0,0,0)),w}function c(p,f){let _=new Ct(p,f);return _.target.copy(new k(0,0,0)),_.update(),_}function a(){let p=new URL("/assets/venice_sunset_1k.0e72ed46.hdr",self.location).href,f=new Ee;return f.background=new H("#000000"),f.environment=new Ln().load(p),f.environment.mapping=Ot,f}function u(p){const f=new de(1,100,100);return new Z(f,p)}function d(p){const f=new de(1,100,100);return new Z(f,p)}function l(p){let f=new Ce(new H("#ffffff"),1);f.castShadow=!0,f.intensity=1,p.add(f)}function h(p){const f=new de(1.2,100,100);return new Z(f,p)}function m(p){let f=100,_=new te,T=[new Fe().load(new URL("/assets/cloud.76af7501.png",self.location).href)],w=1;for(let x=0;x<f;x++)for(let N=0;N<1;N++){let C=new Ft(new Pt({map:T[N],transparent:!0,opacity:.5}));C.position.set((Math.random()*2-1)*(w+Math.random()*10),10+Math.random()*(10/5),(Math.random()*2-1)*(3+Math.random()*10)),C.scale.set(5,5,5),w+=.2,_.add(C)}p.add(_)}function A(p){let f=[new URL("/assets/posx.0b453472.jpg",self.location).href,new URL("/assets/negx.ba52c8a0.jpg",self.location).href,new URL("/assets/posy.106f8c18.jpg",self.location).href,new URL("/assets/negy.aebb71ea.jpg",self.location).href,new URL("/assets/posz.46bd2041.jpg",self.location).href,new URL("/assets/negz.8a623aca.jpg",self.location).href],_=new kt;p.background=_.load(f)}let E,R,y,M;function g(p,f){const _=new URL("/assets/people.df1342b1.glb",self.location).href;new Wt().load(_,w=>{E=w.scene,E.castShadow=!0,R=w.animations,p.add(E),M=new Dt(E),f.push(M),y=M.clipAction(R[0]),y.play()})}function v(p){let f=new Gt,_=new U(p.clientX/window.innerWidth*2-1,-(p.clientY/window.innerHeight)*2+1),T=new k(0,1,0),w=new Ut(T,0);f.setFromCamera(_,e);let x=f.ray.intersectPlane(w,T);if(!x)return;let N=new k(E.position.x,0,E.position.z),C=new k(x.x,0,x.z),F=N.distanceTo(C);E.lookAt(x.x,0,x.z),y.getClip()!=R[1]&&(y.fadeOut(.2),y=M.clipAction(R[1]),y.reset().setEffectiveTimeScale(1).setEffectiveWeight(2).fadeIn(.2).play()),r&&L.remove(r),r=new L.Tween(N).to(C,F*800).easing(L.Easing.Linear.None).onComplete(function(){y.fadeOut(.2),y=M.clipAction(R[0]),y.reset().setEffectiveTimeScale(1).setEffectiveWeight(2).fadeIn(.2).play()}).onUpdate(function(){E.position.set(this._object.x,.04,this._object.z);let I=E.position.clone();I.y=1,t.target=I,t.update()}).start()}function b(p){let f=p.domElement,_=window.devicePixelRatio,T=f.clientWidth*_|0,w=f.clientHeight*_|0,x=f.width!==T||f.height!==w;return x&&p.setSize(T,w,!1),x}return(p,f)=>(qt(),Kt("div",On,[Xt("canvas",Fn,null,512)]))}});var Un=jt(Pn,[["__scopeId","data-v-4ff3c67b"]]);export{Un as default};
