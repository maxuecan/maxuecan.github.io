...
class workspace {
  constructor () {
    ...
    this.put_file_select.onchange = () => {
      Files.handle_put_file_select ()
      Code.tabClick('files')
    };

    this.open_example = get('#open_example')
    this.example_container = get('#example-container')
    this.open_example.onclick = () => {
      this.example_container = get('#example-container')
      this.example_container.style.display = 'block'
    }
...
workspace.prototype.change = function () {
  ...
  //if (this.selector.value in this.devices) {
    ...
    let selected = this.devices ['RPI_Pico'];
...
workspace.prototype.saveXML = function (uid) {
  ...
  element.setAttribute('download', 'main.xml'),
