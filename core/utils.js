  ...
  */
  static makeAName (code, ext) {
    ...
    if (ext == '') {
      ...
      return desc ? `${desc [1].replaceAll(' ', '_').replaceAll('.', '').slice().substring(0,30)}.${ext}` : imp.length ? `my_${imp.slice(-1)[0][1]}_project.${ext}` : `main.${ext}`;
    }
    */
...
class files {
  ...
  resize () {
    ...
    this.editor.setSize(window.innerWidth - (10*$em), window.innerHeight - (4*$em));
  }
  /**
  ...
  libs_save_as() {
    if(this.put_file_name == null || this.put_file_name == "")
      return;

    var codeStr = Files.editor.getDoc().getValue("\n");
    this.putCode(codeStr);
  }

  libs_file_save(libName, codeStr) {
    //Files.editor.getDoc().setValue(codeStr);
    this.put_file_name = libName;
  ...
  directlyReadDevCode() {
    mux.bufferPush ('import os; os.listdir()\r', files.readDevCode.bind(this));
    mux.bufferPush ('import os; os.listdir()\r', files.readDevCode.bind(this));
  }

  runDevMainCode () {
    mux.bufferPush ('import os; os.listdir()\r', files.runDevCode.bind(this)); 
    mux.bufferPush ('import os; os.listdir()\r', files.runDevCode.bind(this)); 
  }
  ...
  handleCurrentProject () {
    ...
    if (this.file_save_as.className == 'bipes-py' || this.file_save_as.className == "py")
      this.internalPython ();
