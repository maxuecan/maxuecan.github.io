...
Code.TABS_ = ['blocks', 'files', 'console'];
...
Code.addEventHandlers = function() {

  let toolbutton = document.getElementById("toolsButton");
  ...
  for (let i = 0; i < dropdown.children.length; i++) {
    let item = dropdown.children[i];
    item.addEventListener("click", function() {
      // 先确保是在代码页面
      const tab_files = document.getElementById("tab_files");

      if (tab_files.classList.contains('tabon')) {
        switch (item.getAttribute("id")) {
          case "save_py_to_dev_main":
            Files.file_save_as.className = 'py';  
            Files.files_save_as();
            break;
          case "save_py_to_dev_lib":
            Files.file_save_as.className = 'py';  
            Files.libs_save_as();
            break;
          case "read_dev_py":
            Files.directlyReadDevCode();
            break;
          case "run_dev_py":
            Files.runDevMainCode();
            break;
          case "save_py_to_local":
            Files.saveCode2Local();
            break;
          default:
            break;
        } 
      ...
      if (item.id && item.id === "new-project") {
        newProjectHandler()
      }
    });
  }
...
function newProjectHandler() {
  const result = window.confirm('请检查程序是否保存！确认执行将创建新工程');
  if (result) {
    // 执行确认后的操作
...
$(document).ready(function() {
  $("#jstree1")
    ...
    $('#jstree1').on('select_node.jstree', function(e, data) {
        let ref = $("#jstree1").jstree(true)
        let source = ref._model.data
        let { parents, text } = data.node
        if (text.indexOf('.xml') !== -1) {
          if (parents.length > 0) {
            let parents_reverse = parents.reverse()
            let path = ''
            parents_reverse.forEach((item, index) => {
              if (index !== 0) {
                path += source[item].text + '/'
              }
            })
            path += text
            loadExampleFromURL(path)
          }
        }
    });
...
function createLibList() {
  for (let i = 0; i < libs_source.length; i++) {
    let dom = `
      <li>
        <span>${libs_source[i]}</span>
        <button onclick="downloadLib('${libs_source[i]}')">下载</button>
      </li>
    `
    $('.lib-content').append(dom)
  }
}
function downloadLib(name) {
  console.log(name)
}
